  ...
  useEffect(() => {
    let ctx = gsap.context(() => {
      ...
      ScrollTrigger.create({
        trigger: ".features-container",
        start: "top top",
        end: `+=${totalCards * 100}vh`, // 100vh per card
        pin: true,
        scrub: 1,
  ...
  return (
    <section className="bg-black pt-16 sm:pt-20 text-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Features Container */}
        <div className="relative features-container h-[90vh] w-full max-w-6xl mx-auto overflow-hidden">
          {cards.map((card, i) => (
            <div
              key={i}
              className="feature-card absolute inset-0 flex items-center justify-center"
